...
void addObjectUI(shared_ptr<IEstateService> service) {
    ...
    cout << "Введіть назву: ";   getline(cin >> ws, name);
    cout << "Введіть адресу: ";  getline(cin >> ws, addr);
    cout << "Введіть площу: ";   area  = (double)trueint();
    cout << "Введіть ціну: ";    price = (double)trueint();
    cout << "Введіть ціну оренди: "; rent = (double)trueint();

    if (type == 1) {
        cout << "Введіть поверх: ";            floor = trueint();
        cout << "Введіть к-сть кімнат: ";      rooms = trueint();
        cout << "Введіть дату реєстрації: ";   cin >> date;
        service->addProperty(new Apartment(name, addr, area, price, rent, floor, rooms, date));
    } else if (type == 2) {
        cout << "Введіть тип бізнесу (напр. IT, Магазин): "; cin >> bType;
        service->addProperty(new Commercial(name, addr, area, price, rent, bType));
    } else if (type == 3) {
        cout << "Введіть площу ділянки (соток): "; land = (double)trueint();
        cout << "Чи є гараж? (1 - Так, 0 - Ні): "; garageInput = trueint();
    ...
    while (choice != 0) {
        ...
        switch (choice) {
            ...
            case 2:
                ...
                break;
            case 3:
                cout << "Введіть назву для видалення: "; cin >> name;
                service->deleteProperty(name);
                break;
            case 4:
                cout << "Введіть назву об'єкта для ремонту: "; cin >> name;
            ...
            case 5:
                ...
                break;
            case 6:
                cout << "Введіть назву об'єкта: "; cin >> name;
                service->toggleRent(name);
                break;
            case 7:
                cout << "Введіть назву для купівлі: "; cin >> name;
            ...
            case 8:
                ...
                break;
            case 9:
                cout << "Введіть назву для продажу: "; cin >> name;
                service->sellProperty(name);
                break;
            case 10:
                cout << "Загальна кількість об'єктів: "
                     << Property::totalObjects << endl;
                break;
            case 11:
